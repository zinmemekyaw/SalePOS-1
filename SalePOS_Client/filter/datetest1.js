...
$(document).ready(function() {
  $.ajax({
      ...
      //   var ul= $('<ul>');
      //   $(ul).append("<li>" + data.results[i].transaction_date+"</li>");
      //   $(ul).append("<li>" + data.results[i].amount_paid+"</li>");
      //   $(ul).append("<li>" + data.results[i].paid+"</li>");
      //   $('#view_rawData').append(ul);
      // }

  $('.datepicker').datepicker(); //Activates the datepicker Materialize CSS element

  preview_json(data.results, $("#view_rawData")); //show the original data on the screen
  $("#count_RawData").html("(" + data.results.length + ")");
  ...
  $("#filter-button").click(function(e) {
    ...
    var filteredData = dateFilterer(data.results, getFormData.dates.start, getFormData.dates.end);
...
function dateFilterer(sourceValues, startDate, endDate) {
  ...
  function filterFunction_dates(sourceValue) {
    console.log("this is from filter date")
    console.log(sourceValue)
    //Aim: To test if the tested date is valid within a particular range
    var rangeAcceptance = {
      minValid: new Date(sourceValue.transaction_date) >= new Date(startDate),
      maxValid: new Date(sourceValue.transaction_date) <= new Date(endDate)
    };
